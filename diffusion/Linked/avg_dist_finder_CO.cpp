...
float dist(struct atom a1, struct atom a2) {
  ...
  yd = a1.y - a2.y;
  zd = a1.z - a2.z;
  float d = 0, h = -30.180556859553505+59.775441140450276;
  if (2 * abs(xd) > h) {
    xd = xd - (sgn(xd) * h);
  }
  if (2 * abs(yd) > h) {
...
int main() {
  ...
  for (int l = 0; l < findAtoms.size(); l++) {
    for (int i = 0; i < atoms.size(); i++) {
      //if(atoms[i].type==15)cout<<"Found atom "<<atoms[i].atomNo<<"bond "<<findAtoms[l].first<<" "<<findAtoms[l].second<<endl;
      if (findAtoms[l].first == atoms[i].atomNo && (atoms[i].type == 17 || atoms[i].type == 16)) {
        for (int j = 0; j < atoms.size(); j++) {
          if (findAtoms[l].second == atoms[j].atomNo) {
            cnt++;
            cout << atoms[i].atomNo << " " << atoms[j].atomNo << " "<<sqrt(dist(atoms[i], atoms[j]))<<endl;
            if(atoms[i].type ==16){
              cout<<"FOUND "<<atoms[i].atomNo<<" "<< sqrt(dist(atoms[i], atoms[j]))<<endl;
            }
