...
float dist(struct atom &a1, struct atom &a2)
{
    ...
    yd = a1.y - a2.y;
    zd = a1.z - a2.z;
    float d = 0, h = -28.716669880139193 + 61.67233011986395;
    if (2 * abs(xd) > h)
    {
        xd = xd - (sgn(xd) * h);
    }
...
void generateOutputFiles()
{
    ...
    bout.open("anglesOutput_topo.txt");
    ...
    bout.open("PVA4_GA20_NC_CROSSLINKED_final_topo.data");
    bout << "Atoms" << endl;
    bout << "" << endl;
    for (int i = 0; i < atoms.size(); i++)
    {
...
bool checkBondFormation(struct atom &a1, struct atom &a2)
{
    return 1;
    float x1, x2, y1, y2, z1, z2, x3, y3, z3, dist1, lambda, dist2, P = 45;
    x1 = a1.x;
    x2 = a2.x;
    y1 = a1.y;
    y2 = a2.y;
    z1 = a1.z;
    z2 = a2.z;
    lambda = -((x1 - P) * (x2 - x1) + (y1 - P) * (y2 - y1) + (z1 - P) * (z2 - z1));
    lambda /= pow((x2 - x1), 2) + pow((y2 - y1), 2) + pow((z2 - z1), 2);
    x3 = x1 + lambda * (x2 - x1);
    y3 = y1 + lambda * (y2 - y1);
    z3 = z1 + lambda * (z2 - z1);
    dist2 = pow((x3 - P), 2) +
    ...
    dist1 = pow((P - y1) * (z2 - z1) - (P - z1) * (y2 - y1), 2) + pow((P - x1) * (z2 - z1) - (P - z1) * (x2 - x1), 2) + pow((P - x1) * (y2 - y1) - (P - y1) * (x2 - x1), 2);
    dist1 /= pow((x2 - x1), 2) + pow((y2 - y1), 2) + pow((z2 - z1), 2);
    dist1 = sqrt(dist1);
    dist2 = sqrt(dist2);
    cnt++;
    if (dist2 <= 10)
    ...
    cout << "M1 " << dist2 << " M2 " << dist1 << endl;
    return (dist2 > 10.1 || (x3 >= min(x1, x2) && x3 <= max(x1, x2) && y3 >= min(y1, y2) && y3 <= max(y1, y2) && z3 >= min(z1, z2) && z3 <= max(z1, z2)));
}
int main()
{
    getAtoms();
    ...
    for (int i = 0; i < atoms14.size(); i++)
    {
        ...
        for (int j = 0; j < atoms11.size(); j++)
        {
            ...
            if (((atoms14[i].type == 14) && (atoms11[j].type == 11) && dist(atoms14[i], atoms11[j]) <= cutoffDist * cutoffDist) && checkBondFormation(atoms14[i], atoms11[j]))
            {
            ...
            else if ((atoms14[i].type == 19) && (atoms11[j].type == 11) && dist(atoms14[i], atoms11[j]) <= cutoffDist * cutoffDist && checkBondFormation(atoms14[i], atoms11[j]))
            {
